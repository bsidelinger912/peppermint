<script lang="ts">
  ...
  const { data: songs } = collectionQuery<Song>(query(collection(db, "songs"), where("id", "in", album.songs)))
  const {data: artist } = documentQuery<Artist>(doc(db, "artists", album.artist));
...
<MenuCard title={album.name} image={album.image} subTitle={$artist?.name}>
